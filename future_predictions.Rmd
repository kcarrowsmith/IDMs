...
# Projections
This is a for-loop that projects everything. It is not finished yet.
```{r predict}
...
```
This currently runs for one, so I need to get it to save somewhere useful so that I can run it for everything. And also figure out how I want to compare the overlaps. Simple subtraction?

```{r overlap - IDM}
overlap.overestimate = overlap.predict - idm.predict
```
